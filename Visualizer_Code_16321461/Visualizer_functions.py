...
def station_with_sensor(frame, sensor, start, end, graph_type):
    ...
    s = "generated_data\\sensor_id_" + str(sensor) + ".csv" # reads sensor file
    df = pd.read_csv(s)  # dataframe
...
def multi_plot2(frame, sensor, sensor2, start, end, graph_type):
    ...
    df = pd.read_csv(s) #read first sensor csv
    ...
    s2 = 'generated_data\\sensor_id_' + str(sensor2) + '.csv'
    df2 = pd.read_csv(s2) #read the second sensor csv
...
def multi_plot3(frame, sensor, sensor2, sensor3, start, end, graph_type):
    ...
    #read from sensor 3
    s3 = 'generated_data\\sensor_id_' + str(sensor3) + '.csv'
    df3 = pd.read_csv(s3) 
...
def multi_plot4(frame, sensor, sensor2, sensor3, sensor4, start, end, graph_type):
    ...
    #read from sensor 1
    s4 = 'generated_data\\sensor_id_' + str(sensor4) + '.csv'
    df4 = pd.read_csv(s4) 
...
def bar_plot(frame, sensor, bar_type):
    ...
    df = pd.read_csv('generated_data\\sensor_id_' + str(sensor) + '.csv') #reads from selected sensor number
...
def bar_plot_h(frame, sensor, bar_type):
    ...
    top = tk.Toplevel(frame)
    df = pd.read_csv('generated_data\\sensor_id_' + str(sensor) + '.csv')
